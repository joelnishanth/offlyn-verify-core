...
\author{Joel Nishanth Ponukumatla and Rahul Natarajan\\Offlyn.ai\\\texttt{hi@offlyn.ai}}
\date{Draft v0.3 -- June 22, 2026}
...
\section{Hardware Reference Artifact: SilicaFold}

SilicaFold is a TinyTapeout-scale hardware prototype that demonstrates a narrow, deterministic policy-gate primitive for structured agent actions~\cite{silicafold2026}. It is not the full \ovc system. Instead, it provides hardware-backed evidence that a policy-at-actuation-boundary decision primitive can be represented in RTL, validated with reproducible tests, and prepared for open silicon workflows. \ovc extends this primitive into a broader architecture involving signed policies, context attestation, authenticated command channels, audit integrity, runtime normalization, and human-in-the-loop escalation.

\subsection{Boundary of Claims}

The SilicaFold V0 artifact should be interpreted as supporting evidence for feasibility, not as a production security implementation. It does not implement cryptographic policy verification, secure boot, authenticated pins, replay protection, tamper resistance, or append-only audit digests. These capabilities are part of future \ovc architecture and patent/paper work. SilicaFold demonstrates that part of the policy-at-actuation-boundary concept can be represented as deterministic hardware; it does not demonstrate the full VerifyCore enforcement stack or production security properties.
...
The \ovc artifact is publicly available at:
\begin{center}
\url{https://github.com/joelnishanth/offlyn-verify-core}
\end{center}

\noindent The SilicaFold hardware reference artifact is available at:
\begin{center}
...
\bibitem{silicafold2026}
J. N. Ponukumatla and R. Natarajan.
\newblock SilicaFold: A TinyTapeout Hardware Primitive for Policy-Gated Offline AI Actuation.
\newblock Draft / forthcoming hardware-backed data paper, Offlyn.ai, 2026.
\newblock \url{https://github.com/offlyn-ai/silicafold-offlyn.ai-chip}
