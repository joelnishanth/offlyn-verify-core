...
\title{Policy at the Actuation Boundary:\\Hardware-Enforced Safety for AI-Controlled Robots}
\author{Author Name(s)\\Offlyn Verify Core Project\\\texttt{author@example.com}}
\date{Draft v0.1 -- June 16, 2026}
...
The gap is not absence of safety checking. The gap is the trust placement of the final action authority. Table \ref{tab:related} summarizes the distinction.
...
\ovc & Hardware-rooted authorization for AI-originated robotic action requests & Yes & Deterministic policy gate & Yes & Actuation boundary \\
\bottomrule
\end{tabularx}
\end{table*}

The proposed contribution is the combination of: (i) AI-originated action requests, (ii) a canonical action authorization interface, (iii) signed policy capsules, (iv) hardware-rooted fail-closed release of actuator-bound commands, and (v) evidence records for each allow or deny outcome. To the best of our current literature scan, this exact systems framing is not yet established as a peer-reviewed robotics paper. The closest research neighbors are ZTPM, SafeGate, ROSClaw, EHV, Simplex/RTA, and Jiao; the proposed design should therefore be presented as an integration and boundary-placement contribution rather than as a claim that policy enforcement or robot safety monitoring is itself novel.
...
The prototype artifact accompanying this paper is publicly available at:
\begin{center}
\texttt{https://github.com/joelnishanth/offlyn-verify-core}
\end{center}

\noindent The repository includes the draft paper source, signed policy examples, a deterministic Verify Core gate simulator, robot actuator simulators, scenario runners, audit logging, and a pytest suite covering safe actions, unsafe commands, unsigned policy updates, rollback attempts, replay attacks, forged tokens, and direct actuator bypass attempts. All tests and scenarios can be reproduced with standard Python tooling and no external services.
